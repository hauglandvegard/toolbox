...
def generate_random_user_agent(
    ...
    match browser_type:
        ...
        case "firefox":
            firefox_versions = list(
                range(firefox_versions[0], firefox_versions[1])
            )  # Last 10 versions of Firefox
            browser_version = random.choice(firefox_versions)
        case _:
            raise ValueError(f"Unsuported browsertype '{browser_type}'")
    ...
    if device_type == "android":
        ...
        base = f"Mozilla/5.0 (iPhone; CPU iPhone OS {ios_version.replace('.', '_')} like Mac OS X) AppleWebKit/"

        if browser_type == "chrome":
            return (
                base
                + f"537.36 (KHTML, like Gecko) CriOS/{browser_version} Mobile/15E148 Safari/604.1"
            )
        elif browser_type == "firefox":
            return (
                base
                + f"605.1.15 (KHTML, like Gecko) FxiOS/{browser_version}.0 Mobile/15E148 Safari/605.1.15"
            )
